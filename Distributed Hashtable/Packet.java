...
public class Packet {
	...
	public boolean unpack(byte[] buf, int bufLen) {
		...
		for (int i = 1; i < lines.length; i++) {
			...
			// bold
                ...
                String ip = chunks[0];
                int port = Integer.parseInt(chunks[1]);
                relayAdr = new InetSocketAddress(ip, port);
            } else if (left.equals("hashRange")) {
                chunks = right.split(":");
                if (chunks.length != 2) return false;
                hashRange = new Pair<Integer,Integer> (Integer.parseInt(chunks[0]),
                        Integer.parseInt(chunks[1]));
	...
	public String toString() {
        ...
        if (clientAdr != null) {
            s.append("clientAdr:");s.append(clientAdr.getAddress().getHostAddress());
            s.append(":"); s.append(clientAdr.getPort()); s.append("\n");
        }
        //end bold
		if (relayAdr != null) {
			s.append("relayAdr:");
        ...
        //bold
        if (succInfo != null) {
            s.append("succInfo:"); s.append(succInfo.left.getAddress().getHostAddress());
            s.append(":"); s.append(succInfo.left.getPort()); s.append(":");
            s.append(succInfo.right);s.append("\n");
        }
        if (predInfo != null) {
            s.append("predInfo:"); s.append(predInfo.left.getAddress().getHostAddress());
            s.append(":"); s.append(predInfo.left.getPort()); s.append(":");
            s.append(predInfo.right);s.append("\n");
        }
        //bold end
		if (senderInfo != null) {
			s.append("senderInfo:");
